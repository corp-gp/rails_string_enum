...
module PgEnumMigrations
  ...
  def add_enum_value(enum_name, value, before: nil, after: nil, schema: 'public')
    opts =
      if before then "BEFORE #{escape_enum_value(before)}"
  ...
  private def enum_name(name, schema)
    [schema || 'public', name].map { |s|
      %("#{s}")
    }.join('.')
  end

  private def escape_enum_value(value)
    escaped_value = value.to_s.sub("'", "''")
    "'#{escaped_value}'"
  end

  private def escape_enum_values(values)
    values.map { |value| escape_enum_value(value) }.join(',')
  end

  private def convert_to_enum(table, col_name, enum_name, keys, convert_sql, default, use_exist_enum)
